...
CREATE TABLE `bus_timetable` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_index` (`sub_route_id`,`direction`,`service_day`,`departure_time`),
  CONSTRAINT `bus_timetable_ibfk_1` FOREIGN KEY (`sub_route_id`, `direction`) REFERENCES `bus_route` (`sub_route_id`, `direction`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
