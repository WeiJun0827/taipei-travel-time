...
import * as Metro from '../server/models/metro_model';
import * as Bus from '../server/models/bus_model';
...
const importMetroRoute = async function () {
  ...
  for (const routeStation of routeStationData) {
    if (routeStation.Direction === 0) {
      const routeId = routeStation.RouteID;
      const stationArray = routeStation.Stations;
      routeStationInfo[routeId] = {
...
const importMetroSchedule = async function () {
  ...
  for (const line of lines) {
    ...
    for (const station of stations) {
      ...
      for (const travelTime of travelTimes) {
        const availableRoutes = await Metro
          .getCalculatedIntervalByStation(travelTime.from_station_id, travelTime.to_station_id);
